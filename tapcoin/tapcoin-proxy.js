...
class TapCoins {
    ...
    async loadProxies() {
        try {
            ...
            this.log('Cannot read proxy.txt file', 'error');
            this.proxyList = [];
        }
    }
        ...
        if (taskListResult.success) {
            ...
            for (const task of incompleteTasks) {
                ...
                if (completeResult.success) {
                    ...
                    this.log(`Task ${task.title} failed: ${completeResult.error}`, 'error');
                }
            ...
            this.log(`Failed to fetch task list: ${taskListResult.error}`, 'error');
        }
    }
    ...
    async handleMiningUpgrades(token, initialCoin, axiosConfig) {
        ...
        try {
            ...
            this.log('Cannot read config.json. Creating new file with default values.', 'warning');
            config = {
        ...
        this.log(`Current coin: ${currentCoin}`, 'info');
        this.log(`Upgrade cost limit: ${maxUpgradeCost} coins`, 'info');
    
        let allTasks = [];
        for (let categoryId = 1; categoryId <= 4; categoryId++) {
            this.log(`Fetching mining task category ${categoryId}...`, 'info');
            const result = await this.getMiningTasks(token, categoryId, axiosConfig);
            
            if (result.success) {
                allTasks = allTasks.concat(result.data);
            } else {
                this.log(`Error fetching category ${categoryId}: ${result.error}`, 'error');
            }
        ...
        for (const task of allTasks) {
            ...
            if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
                this.log('Reached maximum consecutive failures, pausing upgrade', 'warning');
                break;
            }
    
            this.log(`Upgrading task: ${task.name}`, 'info');
            this.log(`Cost: ${task.upgrade_cost} | Earnings: ${task.upgrade_earnings}`, 'custom');
            
            const upgradeResult = await this.upgradeMiningTask(token, task.id, axiosConfig);
            
            if (upgradeResult.success) {
                currentCoin = upgradeResult.currentCoin;
                this.log(`Upgrade successful for task ${task.name}!`, 'success');
                consecutiveFailures = 0;
            } else {
                this.log(`Upgrade failed for task ${task.name}: ${upgradeResult.error}`, 'error');
                consecutiveFailures++;
    
                if (upgradeResult.error.includes('frequent operations')) {
                    this.log(`Detected frequent operations, increasing delay to ${upgradeDelay * 2}ms`, 'warning');
                    await new Promise(resolve => setTimeout(resolve, upgradeDelay * 2));
                } else if (upgradeResult.error.includes('Insufficient balance')) {
                    this.log('Insufficient balance, pausing upgrade', 'warning');
                    break;
                }
            }
    ...
    async main() {
        ...
        const taskQuestion = await this.askQuestion('Do you want to do tasks? (y/n): ');
        const doTasks = taskQuestion.toLowerCase() === 'y';
        const upgradeQuestion = await this.askQuestion('Do you want to upgrade? (y/n): ');
        const doUpgrades = upgradeQuestion.toLowerCase() === 'y';
        ...
        while (true) {
            for (let i = 0; i < data.length; i++) {
                ...
                if (loginResult.success) {
                    ...
                    this.log(`Name: ${firstName} ${lastName}`, 'custom');
                    this.log(`Token: ${token}`, 'info');
                    this.log(`Coin: ${coin}`, 'success');
                    this.log(`Power: ${power}/${power_max}`, 'warning');

                    if (power > 0) {
                        this.log(`Detected ${power} power, splitting into 10 taps...`, 'info');
                        ...
                        for (let j = 0; j < powerDistribution.length && tapSuccess; j++) {
                            ...
                            this.log(`Tap ${j + 1}: Using ${tapPower} power (Remaining: ${currentPower})`, 'info');
                            ...
                            if (collectResult.success) {
                                ...
                                this.log(`Tap successful! Received ${collectResult.data.earnedCoin} coin`, 'success');
                                this.log(`Remaining power: ${collectResult.data.remainingPower}`, 'warning');
                                
                                const delay = Math.floor(Math.random() * 2000) + 1000;
                                await new Promise(resolve => setTimeout(resolve, delay));
                            } else {
                                this.log(`Tap failed: ${collectResult.error}`, 'error');
                                tapSuccess = false;
                            }
                        }

                        this.log(`Completed tap cycle! Total coin received: ${totalEarnedCoin}`, 'success');
                    }
